...
public class WriteController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			originalFileName = utils.FileUtil.uploadFile(req, saveDirectory);
		}
		catch (Exception e) {
			//파일업로드 시 오류가 발생되면 경고창을 띄운 후 작성페이지로 이동한다.
			//예외 발생의 이유를 확인하기 위해 printStackTrace() 메서드를 사용하는 것이 좋다.
			e.printStackTrace();
			common.JSFunction.alertLocation(resp, "파일 업로드 오류입니다.", "../mvcboard/write.do");
			return;
		}
		...
		if(originalFileName != "") {
			...
			String savedFileName = utils.FileUtil.renameFile(saveDirectory, originalFileName);
		...
		if(result == 1 ) { //글쓰기 성공시 목록 페이지로 이동
			resp.sendRedirect("../space/sub01list.jsp?");
		}
		else { //글쓰기 실패시 쓰기 페이지로 이동
			common.JSFunction.alertLocation(resp, "글쓰기에 실패하셨습니다.", "../mvcboard/write.do");
		}
	}
}
