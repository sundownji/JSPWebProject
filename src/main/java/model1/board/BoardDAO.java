...
public class BoardDAO extends JDBConnect {
	...
	public BoardDAO(ServletContext app) {
		/*
		 * 부모 생성자에서는 application을 통해 web.xml에 직접 접근하여 컨텍스트 초기화 파라미터를 얻어온다.
		 */
		super(app);
	}
	...
	public List<BoardDTO> selectList(Map<String, Object> map) {
		...
		/*
		 * List계열의 컬렉션을 생성한다. 이때 타입 매개변수는 board테이블 을 대상으로 하므로 BoardDTO로 설정한다.
		 */
	...
	public int insertWrite(BoardDTO dto, String tname) {
		...
		try {
			/*
			 * 인파라미터가 있는 동적쿼리문으로 insert문을 작성한다. 게시물의 일련번호는 시퀀스를 통해 자동부여하고, 조회수는 0으로 입력한다.
			 */
			String query = "INSERT INTO " + tname + " ( " + " num, title, content, id, visitcount) " + " VALUES ( "
	...
	public int insertWriteWithFile(BoardDTO dto, String tname) {
		int result = 0;

		try {
			String query = "INSERT INTO " + tname + " ( " + " num, title, content, id, ofile, sfile) " + " VALUES ( "
	...
	public void updateVisitCount(String num, String tname) {
		/*
		 * 게시물의 일련번호를 통해 visitcount를 1 증가시킨다. 해당 컬럼은 number 타입이므로 사칙연산이 가능하다.
		 */
		String query = "UPDATE " + tname + " SET " + " visitcount= visitcount+1 " + " WHERE num=?";
